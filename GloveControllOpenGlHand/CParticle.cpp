...
void CParticle::particleInit(const double* posit)    /*初始位置、初始速度、fitness = 0、个体极值点设为自身、fitness_pbest = -DBL_MAX*/
{
	...
	for (int i = 0; i != dimension; i++)
	{
		...
		if (position[i] < rangeLow[i])
		{
			...
			position[i] = position[i] + n*len;   /*使得position[i]小于rangeLow[i]，且rangeLow[i]-position[i]<len*/
			position[i] = 2 * rangeLow[i] - position[i];  /*反射墙*/
		}
	...
	for (int i = 0; i != dimension; i++)
		velocity[i] = (2 * ((double)(rand() % 1001) / 1000.0) - 1)*Vmax[i];     //初始速度是[-Vmax,Vmax]闭区间内的均匀采样,论文中采用的方式
...
void CParticle::velocityUpdate(double weight, double factor1, double factor2, const double *gbest)
{
	...
	{
		double r1 = (double)(rand() % 1001) / 1000.0;   /*[0、1]之间的随机数*/
